...
def test_enrich_fills_and_caches():
    ...
    def fake(cards, profile_block):
        ...
        return {i: {"tldr": f"does {c['title']}", "fit": "good entry fit"}
    ...
    # Second time: served entirely from cache, summarizer NOT called again.
    out2 = insights.enrich([_card("1"), _card("2", title="Data Analyst")],
                           "candidate", summarize=fake)
    assert out2[0]["tldr"] == "does Software Engineer"
...
def test_cache_helpers_roundtrip():
    insights._save_cached("greenhouse:9", "a tldr", "a fit")
    got = insights._get_cached(["greenhouse:9", "missing:x"])
    assert got["greenhouse:9"] == {"tldr": "a tldr", "fit": "a fit"}
